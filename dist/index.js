...
class Conjunction extends Conditions {
  ...
  toString() {
    return this.args.length ? this.args.map(arg => "(" + arg + ")").join(" and ") : "";
  }
...
class InclusionOperator extends Condition {
  ...
  toString() {
    return [this.column, this.operator, "(", Array.isArray(this.value) ? this.value.map(val => quoteVal(val)).join(',') : this.value, ")"].join('');
  }
...
class SQLFunction extends SQLObject {
  ...
  toString() {
    return this.name + "(" + this.args.map(arg => arg instanceof SQLObject || (0, _isFinite.default)(arg) ? arg : quoteTerm(arg)).join() + ")";
  }
...
class Select extends Query {
  ...
  toString() {
    ...
    if (this.select_list.length === 0) {
      ...
      select_list = this.select_list.map(c => Array.isArray(c) ? quoteTerm(c[0]) + ' as ' + quoteTerm(c[1]) : quoteTerm(c)).join();
    }

    let from = this.from().map(table => table[0] === table[1] ? quoteTerm(table[0]) : quoteTerm(table[0]) + ' as ' + quoteTerm(table[1]));
    ...
    let groupby = this.aggregations.length ? " group by " + this.aggregations.map(c => quoteTerm(c)).join() : "";
    let having = this.having_conditions.length ? " having " + this.having_conditions : "";
    let order_by = this.order_expressions.length ? "order by " + this.order_expressions.map(e => Array.isArray(e) ? quoteTerm(e[0]) + " " + e[1] : quoteTerm(e)).join() : "";
    let with_totals = this.request_totals ? " with totals " : "";
    let sample = this.sampling ? " sample " + this.sampling : "";
    let limitby = this.limitbycolumns && this.limitbycolumns.columns.length ? " limit " + this.limitbycolumns.limit + " by " + this.limitbycolumns.columns.map(c => quoteTerm(c)).join() : '';
    let limit = this.limits ? " limit " + this.limits.number + (typeof this.limits.offset === "undefined" ? "" : "," + this.limits.offset) : '';
...
const Dialect = (0, _extends2.default)({}, Operators, AggregateFunctions, ArithmeticFunctions, TimeFunctions, Consts, Queries, Utility);
